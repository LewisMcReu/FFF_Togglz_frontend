...
function FlagDesigner({flag}) {
    const chargeEnabled = useFeature(FeatureFlags.CHARGE);
    const colourPicker = useFeature(FeatureFlags.COLOUR_PICKER);
    const nasaEnabled = useFeature(FeatureFlags.NASA);
    const saveEnabled = useFeature(FeatureFlags.SAVE_FLAG);
    const updateEnabled = useFeature(FeatureFlags.UPDATE_FLAG);
